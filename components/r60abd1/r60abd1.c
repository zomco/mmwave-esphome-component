...
int16_t MicRadarR60ABD1::decode_16bit_signed_(uint8_t msb, uint8_t lsb) {
    ...
    if (raw_value & 0x8000) { // Check if the sign bit is set (negative)
        value = -value;
    }
    return value;
}
...
void MicRadarR60ABD1::handle_byte_(uint8_t byte) {
  ...
  if (this->buffer_.size() > MAX_FRAME_LENGTH + 5) { // Allow some margin
  ...
  switch (this->parse_state_) {
    case ParseState::WAITING_HEADER_1:
      ...
      if (!this->buffer_.empty()) this->buffer_.clear();
    ...
    case ParseState::READING_LENGTH_L:
      ...
      if (this->data_length_ > MAX_FRAME_LENGTH) { // Check if data length exceeds reasonable limit
...
void MicRadarR60ABD1::process_frame_(const std::vector<uint8_t> &frame) {
  ...
  switch (control_word) {
    ...
    case CTRL_WORK_STATUS: // 0x05
        ...
        } else if (command_word == 0x81 && length == 1) { // Reply to query
      ...
      switch (command_word) {
          ...
          if (length == 1) {
            ...
            if (this->motion_sensor_ != nullptr) {
             ...
             if (this->motion_text_sensor_ != nullptr) {
                switch(motion_state) {
                    case 0: this->motion_text_sensor_->publish_state("无"); break;
                    case 1: this->motion_text_sensor_->publish_state("静止"); break;
                    case 2: this->motion_text_sensor_->publish_state("活跃"); break;
                    default: this->motion_text_sensor_->publish_state("未知"); break;
                }
            }
          }
          break;
        ...
        case CMD_DISTANCE_REPORT: // 0x04
          if (length == 2 && this->distance_sensor_ != nullptr) {
            ...
            if (distance <= 65530) { // Assuming 65535 is invalid/max range
        ...
        case CMD_POSITION_REPORT: // 0x05
           if (length == 6) {
               ...
               if (this->position_x_sensor_ != nullptr) this->position_x_sensor_->publish_state(x);
               if (this->position_y_sensor_ != nullptr) this->position_y_sensor_->publish_state(y);
               if (this->position_z_sensor_ != nullptr) this->position_z_sensor_->publish_state(z);
           }
           break;
        // Add cases for query replies (0x81, 0x82, 0x83, 0x84, 0x85) if needed to update state after query
      }
      break;
      ...
      switch (command_word) {
        case CMD_HEART_RATE_VALUE_REPORT: // 0x02
          if (length == 1 && this->heart_rate_sensor_ != nullptr) {
            ...
            if (data[0] > 0 && data[0] < 200) { // More realistic check
          ...
          break;
         // Add cases for query replies (0x82) if needed
         // Waveform data (0x05) is ignored
      }
      break;
      ...
      switch (command_word) {
        case CMD_RESPIRATION_INFO_REPORT: // 0x01
          if (length == 1 && this->respiration_info_sensor_ != nullptr) {
             ...
             switch(data[0]) {
                 case 0x01: info_str = "正常"; break;
                 case 0x02: info_str = "呼吸过高"; break;
                 case 0x03: info_str = "呼吸过低"; break;
                 case 0x04: info_str = "无"; break;
             }
        ...
        case CMD_RESPIRATION_VALUE_REPORT: // 0x02
          if (length == 1 && this->respiration_rate_sensor_ != nullptr) {
            ...
            if (data[0] > 0 && data[0] < 50) { // More realistic check
          ...
          break;
         // Add cases for query replies (0x81, 0x82) if needed
         // Waveform data (0x05) is ignored
      }
      break;
        ...
        switch (command_word) {
            ...
            case CMD_SLEEP_STAGE_REPORT: // 0x02
                 if (length == 1 && this->sleep_stage_sensor_ != nullptr) {
                     ...
                     switch(data[0]) {
                         case 0x00: stage_str = "深睡"; break;
                         case 0x01: stage_str = "浅睡"; break;
                         case 0x02: stage_str = "清醒"; break;
                         case 0x03: stage_str = "无"; break;
                     }
                     ESP_LOGD(TAG, "Sleep stage report: %s", stage_str.c_str());
                     this->sleep_stage_sensor_->publish_state(stage_str);
                 }
                 break;
             case CMD_SLEEP_SCORE_REPORT: // 0x06
                 if (length == 1 && this->sleep_score_sensor_ != nullptr) {
                     ESP_LOGD(TAG, "Sleep score report: %d", data[0]);
                     this->sleep_score_sensor_->publish_state(data[0]);
                 }
                 break;
             // Add more sleep monitor cases as needed (0x0C, 0x0D, 0x0E, 0x10, 0x11, 0x12...)
        }
        break;
...
void MicRadarR60ABD1::send_command(uint8_t control_word, uint8_t command_word, const std::vector<uint8_t>& data_payload) {
   ...
   frame.push_back((data_payload.size() >> 8) & 0xFF); // Length H
   frame.push_back(data_payload.size() & 0xFF);      // Length L
   frame.insert(frame.end(), data_payload.begin(), data_payload.end()); // Data

   // Calculate checksum on the frame *before* adding checksum and footer
   // Checksum includes: Header(2)+Ctrl(1)+Cmd(1)+Len(2)+Data(N)
   ...
   ESP_LOGD(TAG, "Sent frame: %s", format_hex_pretty(frame).c_str());
}
...
//     // Ensure it's a multiple of 10? The protocol doesn't explicitly say it rejects others, but implies steps of 10.
//     // minutes = (minutes / 10) * 10; // Force step of 10? Or let the device handle it? Let's send the value.
//     ESP_LOGD(TAG, "Setting unattended time: %d minutes", minutes);
...
//     this->send_command(CTRL_SLEEP_MONITOR, CMD_SLEEP_STRUGGLE_SENSITIVITY_SET, {level});
// }
